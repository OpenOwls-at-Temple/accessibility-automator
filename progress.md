...
- [x] Opened PR for branch `fix/test-suite-issues` (3 bug fixes: missing reportlab, masked cookie test, title-placeholder crash) — 2026-07-01
- [x] F11: per-file progress tracking — `current_file` on Job, UI shows which file is being processed + row highlight — 2026-07-01
- [x] F12: human-in-the-loop AI review — scan endpoint generates draft suggestions, ReviewModal lets user edit before apply, overrides flow through pipeline → fixer — 2026-07-01
- [x] Fixed: filenames with spaces were rejected by `_SAFE_NAME` regex — added space to allowed chars — 2026-07-01
...
- **2026-07-01 (Phase 2: F11 + F12)** — Implemented F11 (batch per-file progress) and F12 (human-in-the-loop review). F11: added `current_file` field to Job dataclass + JobOut schema; backend sets it before processing each file; frontend FileExplorer shows "Processing: filename.pptx (2 of 9)" live during a fix job and highlights the active row. Button renamed "Fix All". F12: added `scan_file()` to `remediator/pipeline.py` — audits the file and generates AI draft suggestions (alt text via `provider.caption_image`, slide titles via `provider.suggest_title`) without writing anything; new `POST /{group}/files/{name}/scan` endpoint; new `POST /{group}/files/{name}/apply-review` endpoint takes user-approved suggestions, passes them as `overrides` dict through `JobManager` → `remediate_file` → `fix_pptx` → `ai_fixer` (both `fix_image_alt_text` and `fix_slide_title` now accept `override` param that bypasses LLM); new `ReviewModal.jsx` component (scan → edit → apply flow); "Review AI" button added per PPTX file in FileExplorer. Also fixed: `_SAFE_NAME` regex now allows spaces in filenames. pdf_fixer.fix_pdf updated to accept (and ignore) overrides param for interface consistency. All 41 tests still pass.
